...
@pytest.mark.asyncio
async def test_health_returns_healthy_when_dependencies_ok(app_client: AsyncClient) -> None:
    with (
...
@pytest.mark.asyncio
async def test_health_returns_degraded_when_postgres_fails(app_client: AsyncClient) -> None:
    with (
