...
<p align="center">
  <img src="https://img.shields.io/badge/状态-开发中-yellow?style=flat" alt="Status">
...
Dense Embedding（OpenAI）+ BM25 稀疏检索双重召回，可选 Cohere Rerank 精排。兼顾语义理解和关键词精确匹配。
...
**📊 RAGAS Benchmark（Baseline 检索 · 21 QA Pairs）**

| 指标 | 分数 | 说明 |
|:-----|:----:|------|
| **Faithfulness** | **0.7655** | 回答忠实度 — 生成内容是否基于上下文 |
| Answer Relevancy | 0.1538 | 回答相关性 — 答案与问题的匹配度 |
| Context Precision | 0.1667 | 上下文精确率 — 检索到的内容是否相关 |
| Context Recall | 0.1579 | 上下文召回率 — 相关内容是否被检索到 |

> **注：** 本轮使用 DeepSeek Chat + SiliconFlow BGE-M3。`answer_relevancy` / `context_*` 偏低受限于 DeepSeek 不支持 `n>1` 多采样，RAGAS 部分 metric 依赖多次生成计算方差。切换到 GPT-4o 预期显著提升。
...
| Embedding 超时 | 检查 `OPENAI_API_KEY` 余额和网络连通性 |
| Rerank 不生效 | 确认 `COHERE_API_KEY` 已配置，否则自动降级为纯向量检索 |
