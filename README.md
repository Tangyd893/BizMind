...
> **注：** Baseline 使用单阶段 Dense 检索；Agent 使用 Hybrid 检索（Dense + BM25 + Cohere Rerank）。DeepSeek 不支持 `n>1` 多采样，部分 RAGAS metric 偏低；`context_precision` 为 0 受限于 RAGAS 评分机制与 DeepSeek `n=1` 限制。切换到 GPT-4o 预期显著提升。
